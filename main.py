...
if __name__ == "__main__":
    ...
    if modo == 'sim':
        ...
        payload_normal = f'{{"temperature":5.5, "pressure":1013, "humidity":50, "gpsLatitude":-26.30, "gpsLongitude":-48.84, "timestamp":{int(time.time())}}}'
        processar_payload_mqtt(payload_normal, True)

        # Situação crítica
        # Temperatura muito alta, umidade alta, GPS falhou (0,0) e o dado é antigo.
        payload_critico = f'{{"temperature":15.0, "pressure":1013, "humidity":75, "gpsLatitude":0.0, "gpsLongitude":0.0, "timestamp":{int(time.time()) - 600}}}'
